    ...
    <div className="flex items-center justify-center mt-10">
      <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-8">
          ...
          {welcomeConfig.map((section, index) => {
            ...
            return (
              <div key={index} className="text-center p-4" role="listitem">
                <div
                  className={`inline-flex items-center justify-center w-12 h-12 ${section.bgColor} rounded-full mb-3`}
                >
                  <IconComponent className={`w-6 h-6 ${section.color}`} aria-hidden="true" />
                </div>
                <h3 className="text-sm font-medium text-gray-900 mb-1">{section.name}</h3>
          ...
          {instructions.map((instruction, index) => (
            <div key={index} className="flex items-start" role="listitem">
              <div
                ...
                aria-hidden="true"
              />
              <p className="text-sm text-gray-700 leading-relaxed text-left">{instruction}</p>
            </div>
          ))}
        </div>
        ...
        <div id="test-overview" className="sr-only">
          Test overview: This test contains 4 sections - Reading (30 minutes), Listening (30
          minutes), Writing (60 minutes), and Speaking (15 minutes). Total duration approximately 2
          hours and 15 minutes. Please review all instructions before proceeding.
        </div>
      </div>
    </div>
  );
