...
export default function Header() {
    ...
    return (
        <header
            ...
            <div className="text-xl font-bold text-blue-700 cursor-pointer" onClick={goToHome}>IdCert</div>
            ...
            <div className="relative" ref={menuRef}>
                {user ? (
                    ...
                    >
                        <span
                            className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center text-lg font-bold text-blue-800">
                            {user.name.charAt(0)}
                        </span>
                        <span className="font-medium">{user.name}</span>
                        <svg className={`w-4 h-4 transition-transform ${open ? "rotate-180" : ""}`} fill="none"
